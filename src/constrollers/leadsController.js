...
const allocateLeads = async (req, res) => {
    ...
    var id = 0;
    var employee = await Users.find()
    for (let i = 0; i < arr.length; i++) {
        let empData = employee
        let verifiedEmp = empData.map((ele)=>{ // Verifying the employee id
            if(ele.id == id)return true
        })
        
        if (verifiedEmp.includes(true)) {
...
const updateStatus = async(req,res)=>{
    const employeeId = req.params.id
    const updatedStatus = await Leads.findOneAndUpdate({employeeId:employeeId}, {status:req.body.status},{new:true})
    res.status(200).send({status:true, message:"Status updated successfully"});
}

const deleteLeads = async(req,res)=>{
    let id = req.body.id
    await Leads.findByIdAndUpdate({id:id}, {isDeleted:true})
